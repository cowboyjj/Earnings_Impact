...
class GA_HyperparameterTuning:
    ...
    def __init__(self, testType, fold_count, number_of_mutated_genes, mutationRate, n_estimators, weight, earlyStoppingRound, featureCount):
...
def PerformHyperparameterTuning(TestType,
    ...
    tuningGA = GA_HyperparameterTuning(TestType, fold_count, number_of_mutated_genes, mutationRate, n_estimators, weight, earlyStoppingRound, len(x_train.columns))
...
#        lastAverageFitness = currentAverageFitness
    ...
    return (tunedParameters, generationIdx, np.min(fitness), fitness_allGenerations)
